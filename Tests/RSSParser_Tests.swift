...
class RSSParser_Tests: XCTestCase {
    ...
    func test_parser_withValidMock_shouldReturnTheRightValues() {
        ...
        let url = URL(fileURLWithPath: mockFileURL!)
        ...
        RSSParser.parseFeedForRequest(request, callback: { (feed, error) -> Void in
            ...
            {
                XCTAssertEqual(testFeed.title, "Swift Blog - Apple Developer")
                if let link = testFeed.link
                {
                    XCTAssertEqual(link.absoluteString, "http://developer.apple.com/swift/blog/")
                }
                XCTAssertEqual(testFeed.feedDescription, "Get the latest news and helpful tips on the Swift programming language from the engineers who created it.")
                
                XCTAssertEqual(testFeed.language, "en-US")
                ...
                if let date = testFeed.lastBuildDate
                {
                    ...
                    //XCTAssertEqual(dateComponent.timeZone!, self.GMT_timeZone)
                }
                else
                {
                ...
                XCTAssertEqual(testFeed.generator, "Custom")
                XCTAssertEqual(testFeed.copyright, "Copyright 2014, Apple Inc.")
                
                XCTAssert((testFeed.items.count == 15), "number of items should be equal to 15")
                
                var myItem: RSSItem = testFeed.items[0]
                
                XCTAssertEqual(myItem.title, "Building  assert()  in Swift, Part 2:  __FILE__  and  __LINE__ ")
                
                if let link = myItem.link
                {
                    XCTAssertEqual(link.absoluteString, "http://developer.apple.com/swift/blog/?id=15")
                }
                else
                {
                ...
                XCTAssertEqual(myItem.guid, "http://developer.apple.com/swift/blog/?id=15")
                ...
                if let date = myItem.pubDate
                {
                    ...
                    XCTAssertEqual(dateComponent.minute, 0)
                    XCTAssertEqual(dateComponent.second, 0)
                    //XCTAssertEqual(dateComponent.timeZone!, self.self.GMT_timeZone)
                }
                else
                {
                ...
                XCTAssertEqual(myItem.guid, "http://developer.apple.com/swift/blog/?id=14")
                
                if let date = myItem.pubDate
                {
                    var dateComponent = self.calendar.dateComponents(self.calendar_flags, from: date)
                    
                    XCTAssertEqual(dateComponent.weekday, 3)
                    XCTAssertEqual(dateComponent.day, 9)
                    XCTAssertEqual(dateComponent.month, 9)
                    XCTAssertEqual(dateComponent.year, 2014)
                    XCTAssertEqual(dateComponent.hour, 11)
                    XCTAssertEqual(dateComponent.minute, 0)
                    XCTAssertEqual(dateComponent.second, 0)
                    //XCTAssertEqual(dateComponent.timeZone!, self.GMT_timeZone)
                }
                ...
                XCTAssertEqual(myItem.guid, "http://developer.apple.com/swift/blog/?id=13")
                
                if let date = myItem.pubDate
                {
                    var dateComponent = self.calendar.dateComponents(self.calendar_flags, from: date)
                    
                    XCTAssertEqual(dateComponent.weekday, 4)
                    XCTAssertEqual(dateComponent.day, 3)
                    XCTAssertEqual(dateComponent.month, 9)
                    XCTAssertEqual(dateComponent.year, 2014)
                    XCTAssertEqual(dateComponent.hour, 9)
                    XCTAssertEqual(dateComponent.minute, 0)
                    XCTAssertEqual(dateComponent.second, 0)
                    //XCTAssertEqual(dateComponent.timeZone!, self.GMT_timeZone)
                }
                ...
                XCTAssertEqual(myItem.itemDescription, " In Swift, a pattern is a way to describe and match a set of values based on certain rules, such as:    All tuples whose first value is 0  All numbers in the range 1...5  All class instances of a certain type   The learning playground linked below includes embedded documentation and experiments for you to perform. Download it for an interactive experience that will give you a jump start into using patterns in your own apps.  This playground requires the latest beta version of Xcode 6 on OS X Mavericks or OS X Yosemite beta. ")
                XCTAssertEqual(myItem.content, "<p>In Swift, a pattern is a way to describe and match a set of values based on certain rules, such as: </p><ul class=\"tight\"><li>All tuples whose first value is 0</li><li>All numbers in the range 1...5</li><li>All class instances of a certain type</li></ul><p>The learning playground linked below includes embedded documentation and experiments for you to perform. Download it for an interactive experience that will give you a jump start into using patterns in your own apps.</p><p>This playground requires the latest beta version of Xcode 6 on OS X Mavericks or OS X Yosemite beta.</p>")
                
                myItem = testFeed.items[3]
                
                XCTAssertEqual(myItem.title, "Optionals Case Study:  valuesForKeys ")
                
                if let link = myItem.link
                {
                    XCTAssertEqual(link.absoluteString, "http://developer.apple.com/swift/blog/?id=12")
                }
                else
                {
                    XCTFail("link shouldn't be nil")
                ...
                XCTAssertEqual(myItem.guid, "http://developer.apple.com/swift/blog/?id=12")
                ...
                if let date = myItem.pubDate
                {
                    ...
                    XCTAssertEqual(dateComponent.weekday, 3)
                    XCTAssertEqual(dateComponent.day, 26)
                    XCTAssertEqual(dateComponent.month, 8)
                    XCTAssertEqual(dateComponent.year, 2014)
                    XCTAssertEqual(dateComponent.hour, 14)
                    XCTAssertEqual(dateComponent.minute, 0)
                    XCTAssertEqual(dateComponent.second, 0)
                    //XCTAssertEqual(dateComponent.timeZone!, self.GMT_timeZone)
                }
                ...
                XCTAssertEqual(myItem.guid, "http://developer.apple.com/swift/blog/?id=11")
                ...
                XCTAssertEqual(myItem.guid, "http://developer.apple.com/swift/blog/?id=10")
                
                if let date = myItem.pubDate
                {
                    var dateComponent = self.calendar.dateComponents(self.calendar_flags, from: date)
                    
                    XCTAssertEqual(dateComponent.weekday, 6)
                    XCTAssertEqual(dateComponent.day, 15)
                    XCTAssertEqual(dateComponent.month, 8)
                    XCTAssertEqual(dateComponent.year, 2014)
                    XCTAssertEqual(dateComponent.hour, 13)
                    XCTAssertEqual(dateComponent.minute, 30)
                    XCTAssertEqual(dateComponent.second, 0)
                    //XCTAssertEqual(dateComponent.timeZone!, self.GMT_timeZone)
                }
                else
                ...
                XCTAssertEqual(myItem.guid, "http://developer.apple.com/swift/blog/?id=9")
                ...
                if let date = myItem.pubDate
                {
                    ...
                    XCTAssertEqual(dateComponent.weekday, 6)
                    XCTAssertEqual(dateComponent.day, 8)
                    XCTAssertEqual(dateComponent.month, 8)
                    XCTAssertEqual(dateComponent.year, 2014)
                    XCTAssertEqual(dateComponent.hour, 11)
                    XCTAssertEqual(dateComponent.minute, 0)
                ...
                XCTAssertEqual(myItem.guid, "http://developer.apple.com/swift/blog/?id=1")
                ...
                if let date = myItem.pubDate
                {
                    ...
                    XCTAssertEqual(dateComponent.weekday, 6)
                    XCTAssertEqual(dateComponent.day, 11)
                    XCTAssertEqual(dateComponent.month, 7)
                    XCTAssertEqual(dateComponent.year, 2014)
                    XCTAssertEqual(dateComponent.hour, 10)
                    XCTAssertEqual(dateComponent.minute, 0)
                    XCTAssertEqual(dateComponent.second, 0)
                ...
                XCTAssertEqual(myItem.itemDescription, " This new blog will bring you a behind-the-scenes look into the design of the Swift language by the engineers who created it, in addition to the latest news and hints to turn you into a productive Swift programmer.  Get started with Swift by downloading  Xcode 6 beta , now available to all Registered Apple Developers for free. The Swift Resources tab has a ton of great links to videos, documentation, books, and sample code to help you become one of the world’s first Swift experts. There’s never been a better time to get coding!  - The Swift Team  ")
                XCTAssertEqual(myItem.content, "<p>This new blog will bring you a behind-the-scenes look into the design of the Swift language by the engineers who created it, in addition to the latest news and hints to turn you into a productive Swift programmer.</p><p>Get started with Swift by downloading <a href=\"http://developer.apple.com/devcenter/download.action?path=/Developer_Tools/xcode_6_beta_3_lpw27r/xcode_6_beta_3.dmg\" alt=\"\">Xcode 6 beta</a>, now available to all Registered Apple Developers for free. The Swift Resources tab has a ton of great links to videos, documentation, books, and sample code to help you become one of the world’s first Swift experts. There’s never been a better time to get coding!</p><p>- The Swift Team </p>")
            }
        })
    ...
    func test_parser_withWordpressMock_shouldReturnTheRightValues() {
        ...
        let url = URL(fileURLWithPath: wordpressMockFileURL!)
        ...
        RSSParser.parseFeedForRequest(request, callback: { (feed, error) -> Void in
            ...
            if let testFeed = feed
            {
                ...
                XCTAssertEqual(testFeed.title, "WordPress.com News")
                if let link = testFeed.link
                {
                    XCTAssertEqual(link.absoluteString, "http://en.blog.wordpress.com")
                }
                XCTAssertEqual(testFeed.feedDescription, "The latest news on WordPress.com and the WordPress community.")
                
                XCTAssertEqual(testFeed.language, "en")
                ...
                if let date = testFeed.lastBuildDate
                {
                    ...
                    XCTAssertEqual(dateComponent.weekday, 6)
                    XCTAssertEqual(dateComponent.day, 3)
                    XCTAssertEqual(dateComponent.month, 10)
                    XCTAssertEqual(dateComponent.year, 2014)
                    XCTAssertEqual(dateComponent.hour, 13)
                    XCTAssertEqual(dateComponent.minute, 49)
                    XCTAssertEqual(dateComponent.second, 47)
                    XCTAssertEqual(dateComponent.timeZone!, self.GMT_timeZone)
                }
                else
                {
                ...
                XCTAssertEqual(testFeed.generator, "http://wordpress.com/")
            
            
                var myItem = testFeed.items[1]
                
                XCTAssertEqual(myItem.title, "Engaged, Inspired, and Ready to Build a Better Web")
                
                if let link = myItem.link
                {
                    XCTAssertEqual(link.absoluteString, "http://en.blog.wordpress.com/2014/09/30/grand-meetup-reflections/")
                }
                else
                {
                    XCTFail("link shouldn't be nil")
                ...
                XCTAssertEqual(myItem.guid, "http://en.blog.wordpress.com/?p=28737")
                ...
                if let date = myItem.pubDate
                {
                    ...
                    XCTAssertEqual(dateComponent.weekday, 3)
                    XCTAssertEqual(dateComponent.day, 30)
                    XCTAssertEqual(dateComponent.month, 9)
                    XCTAssertEqual(dateComponent.year, 2014)
                ...
                XCTAssertEqual(myItem.itemDescription, "One week every year, the entire <a href=\"http://automattic.com\">Automattic</a> staff gets together to connect, work, and laugh. And then, of course, we blog about it! Could you be blogging about your experience with us in 2015?<img alt=\"\" border=\"0\" src=\"http://pixel.wp.com/b.gif?host=en.blog.wordpress.com&#038;blog=3584907&#038;post=28737&#038;subd=en.blog&#038;ref=&#038;feed=1\" width=\"1\" height=\"1\" />")
                XCTAssertEqual(myItem.content, "<p>Automattic is a distributed company &#8212; we all work from wherever we are. Right now, &#8220;where we are&#8221; is 197 cities around the world: New Orleans, USA. Montevideo, Uruguay. Tokyo, Japan. Vilnius, Lithuania.</p>")
                
                XCTAssertEqual(myItem.imagesFromItemDescription.count, 1)
                XCTAssertEqual(myItem.imagesFromItemDescription[0].absoluteString, "http://pixel.wp.com/b.gif")
                
                if let link = myItem.commentsLink
                {
                    XCTAssertEqual(link.absoluteString, "http://en.blog.wordpress.com/2014/09/30/grand-meetup-reflections/#comments")
                }
                else
                {
                    XCTFail("link shouldn't be nil")
                ...
                {
                    XCTAssertEqual(link.absoluteString, "http://en.blog.wordpress.com/2014/09/30/grand-meetup-reflections/feed/")
                }
                else
                {
                    XCTFail("link shouldn't be nil")
                ...
                XCTAssertEqual(myItem.commentsCount, 41)
                
                XCTAssertEqual(myItem.author, "Michelle W.")
                ...
                XCTAssertEqual(myItem.title, "Gmail Password Leak Update")
                
                if let link = myItem.link
                {
                    XCTAssertEqual(link.absoluteString, "http://en.blog.wordpress.com/2014/09/12/gmail-password-leak-update/")
                }
                else
                {
                    XCTFail("link shouldn't be nil")
                ...
                XCTAssertEqual(myItem.guid, "http://en.blog.wordpress.com/?p=28615")
                ...
                if let date = myItem.pubDate
                {
                    ...
                    XCTAssertEqual(dateComponent.weekday, 6)
                    XCTAssertEqual(dateComponent.day, 12)
                    XCTAssertEqual(dateComponent.month, 9)
                    XCTAssertEqual(dateComponent.year, 2014)
                    XCTAssertEqual(dateComponent.hour, 23)
                    XCTAssertEqual(dateComponent.minute, 53)
                    XCTAssertEqual(dateComponent.second, 37)
                    XCTAssertEqual(dateComponent.timeZone!, self.GMT_timeZone)
                }
                else
                {
                ...
                XCTAssertEqual(myItem.itemDescription, "We've taken extra steps to protect WordPress.com members.<img alt=\"\" border=\"0\" src=\"http://pixel.wp.com/b.gif?host=en.blog.wordpress.com&#038;blog=3584907&#038;post=28615&#038;subd=en.blog&#038;ref=&#038;feed=1\" width=\"1\" height=\"1\" />")
                XCTAssertEqual(myItem.content, "<p>This week, a group of hackers <a title=\"Russian Hackers Released Gmail Password List\" href=\"http://time.com/3318853/google-user-logins-bitcoin/\">released a list</a> of about 5 million Gmail addresses and passwords. This list was not generated as a result of an exploit of WordPress.com, but since a number of emails on the list matched email addresses associated with WordPress.com accounts, we took steps to protect our users.</p>")
                ...
                if let link = myItem.commentsLink
                {
                    XCTAssertEqual(link.absoluteString, "http://en.blog.wordpress.com/2014/09/12/gmail-password-leak-update/#comments")
                }
                else
                {
                    XCTFail("link shouldn't be nil")
                ...
                if let link = myItem.commentRSSLink
                {
                    XCTAssertEqual(link.absoluteString, "http://en.blog.wordpress.com/2014/09/12/gmail-password-leak-update/feed/")
                }
                else
                {
                    XCTFail("link shouldn't be nil")
                ...
                XCTAssertEqual(myItem.commentsCount, 147)
                
                XCTAssertEqual(myItem.author, "Daryl L. L. Houston")
                ...
                XCTAssertEqual(myItem.title, "Early Theme Adopters: Eighties")
                
                XCTAssertEqual(myItem.imagesFromContent.count, 7)
                XCTAssertEqual(myItem.imagesFromContent[0].absoluteString, "https://en-blog.files.wordpress.com/2014/09/screen-shot-2014-09-25-at-12-07-46-pm.png")
                XCTAssertEqual(myItem.imagesFromContent[1].absoluteString, "https://en-blog.files.wordpress.com/2014/09/screen-shot-2014-09-25-at-11-17-31-pm.png")
                XCTAssertEqual(myItem.imagesFromContent[2].absoluteString, "https://en-blog.files.wordpress.com/2014/09/screen-shot-2014-09-25-at-12-05-00-pm.png")
                XCTAssertEqual(myItem.imagesFromContent[3].absoluteString, "https://en-blog.files.wordpress.com/2014/09/screen-shot-2014-09-25-at-11-43-22-pm.png")
                XCTAssertEqual(myItem.imagesFromContent[4].absoluteString, "https://en-blog.files.wordpress.com/2014/09/screen-shot-2014-09-25-at-12-18-29-pm.png")
                XCTAssertEqual(myItem.imagesFromContent[5].absoluteString, "https://en-blog.files.wordpress.com/2014/09/screen-shot-2014-09-25-at-12-06-38-pm.png")
                XCTAssertEqual(myItem.imagesFromContent[6].absoluteString, "http://pixel.wp.com/b.gif")
            }
    ...
    func test_parser_withTumblrMock_shouldReturnTheRightValues() {
        ...
        let url = URL(fileURLWithPath: tumblrMockFileURL!)
        let request: URLRequest = URLRequest(url: url)
        let expectation = self.expectation(description: "GET \(url)")
        ...
        RSSParser.parseFeedForRequest(request, callback: { (feed, error) -> Void in
            ...
            if let testFeed = feed
            {
                ...
                XCTAssertEqual(testFeed.title, "Tumblr Engineering")
                if let link = testFeed.link
                {
                    XCTAssertEqual(link.absoluteString, "http://engineering.tumblr.com/")
                }
                XCTAssertEqual(testFeed.feedDescription, "Dispatches from the intrepid tinkerers behind technology at Tumblr.")
                
                XCTAssertEqual(testFeed.generator, "Tumblr (3.0; @engineering)")
            
                var myItem = testFeed.items[0]
                
                XCTAssertEqual(myItem.title, "Sam Giddins: My Summer at TumblrThis summer, I had the immense...")
                
                if let link = myItem.link
                {
                    XCTAssertEqual(link.absoluteString, "http://engineering.tumblr.com/post/98331642904")
                }
                else
                {
                    XCTFail("link shouldn't be nil")
                ...
                XCTAssertEqual(myItem.guid, "http://engineering.tumblr.com/post/98331642904")
                ...
                if let date = myItem.pubDate
                {
                    ...
                    XCTAssertEqual(dateComponent.weekday, 4)
                    XCTAssertEqual(dateComponent.day, 24)
                    XCTAssertEqual(dateComponent.month, 9)
                    XCTAssertEqual(dateComponent.year, 2014)
                    XCTAssertEqual(dateComponent.hour, 16)
                    XCTAssertEqual(dateComponent.minute, 43)
                    XCTAssertEqual(dateComponent.second, 45)
                    XCTAssertEqual(dateComponent.timeZone!, self.DST_timeZone)
                }
                else
                {
                    XCTFail("pubDate shouldn't be nil")
                ...
                XCTAssertEqual(myItem.guid, "http://engineering.tumblr.com/post/98050002584")
                ...
                if let date = myItem.pubDate
                {
                    ...
                    XCTAssertEqual(dateComponent.day, 21)
                    XCTAssertEqual(dateComponent.month, 9)
                    XCTAssertEqual(dateComponent.year, 2014)
                    XCTAssertEqual(dateComponent.hour, 8)
                    XCTAssertEqual(dateComponent.minute, 42)
                    XCTAssertEqual(dateComponent.second, 50)
                    XCTAssertEqual(dateComponent.timeZone!, self.DST_timeZone)
                }
                else
                {
    ...
    func test_parser_withInvalidMock_shouldReturnParsingError() {
        ...
        let url = URL(fileURLWithPath: invalidMockFileURL!)
        let request: URLRequest = URLRequest(url: url)
        let expectation = self.expectation(description: "GET \(url)")
    ...
    func test_parser_withInvalidURL_shouldReturnNetworkError() {
        ...
        let url = URL(string: "file://no/no/no/nothing.rss")!
        let request: URLRequest = URLRequest(url: url)
        let expectation = self.expectation(description: "GET \(url)")
    ...
    func test_parser_withEmptyMock_shouldBehaveProperly() {
        ...
        let url = URL(fileURLWithPath: emptyMockFileURL!)
        let request: URLRequest = URLRequest(url: url)
        let expectation = self.expectation(description: "GET \(url)")
